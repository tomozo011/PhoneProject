    ...
    public function au(Request $request){
        ...
        for($i=1; $i <= $request->member; $i++){
            ...
            if($request->{$GBs[$count]} <= 1){
                $Plans[] = Plan::where('id',7)->first();
            }elseif(1 < $request->{$GBs[$count]} && $request->{$GBs[$count]} <= 4) {
                $Plans[] = Plan::where('id',8)->first();
            }elseif(4 < $request->{$GBs[$count]} && $request->{$GBs[$count]} <=7) {
                $Plans[] = Plan::where('id',9)->first();
            }elseif(8 <= $request->{$GBs[$count]}){
                $Plans[] = Plan::where('id',6)->first();
            }

            $Plans_free = Plan::where('id',6)->first();

            // 通話
            $Tells = Tell::where('carrier', 'au')->get();
            ...
            // カード
            $Cards = Card::where('plan_id', 201)->first();
            ...
            // ネット 
            $Net1 = Net::where('plan_id','201')->first();
            $Net2 = Net::where('plan_id', '202')->first();
            ...
            // 学割
            // $Students = Student::where('carrier', 'au')->first();
...
// dd($answers);
        ...
        return view('phone.au',compact('name','user_id', 'a', 'plansName', 'plansPrice', 'getMem','getNet','getCard', 'getGBs', 'gettells', 'getages', 'getnows', 'Plans_answer', 'Tells_name', 'Tells_price','Nets_price','Member_price','Cards_price', 'Students_price', 'Totals', 'answers', 'auths'));
    }
}
